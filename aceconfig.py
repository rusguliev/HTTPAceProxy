...
class AceConfig(acedefconfig.AceDefConfig):
    ...
    acecmd = "acestreamengine --client-console --live-cache-size 2000000000 --upload-limit 100 --max-upload-slots 5 --live-buffer 80 --cache-dir .AceStreamCache --vod-buffer 40 --api-port=62062 --http-port=6878 --bind 10.88.69.3"
    # Ace Stream API key
    ...
    transcodecmd['hls'] = 'ffmpeg -nostats -loglevel info -i - -map 0 -c copy -force_key_frames "expr:gte(t,n_forced*2)" -f hls -start_number 0 -hls_time 10 -hls_init_time 60 -hls_list_size 10 -hls_playlist_type event -hls_allow_cache 1 -hls_flags single_file+split_by_time+delete_segments+omit_endlist+append_list -'.split()
